    ...
    <div className="min-h-screen bg-gray-100">
      <div className="bg-white shadow-sm p-4 sticky top-0 z-50 flex items-center justify-between">
        ...
        {order.status === 'Processing' && timeLeft > 0 && (
            <button 
                ...
                className="bg-red-50 text-red-600 border border-red-200 hover:bg-red-100 px-3 py-1.5 rounded-lg text-xs font-bold transition flex items-center gap-1"
            >
                <X size={14} /> Cancel ({minutes}:{seconds < 10 ? `0${seconds}` : seconds})
            </button>
        )}
      </div>
      ...
      <div className="container mx-auto p-4 max-w-2xl">
        {order.status === 'Cancelled' ? (
            <div className="bg-red-50 border-l-4 border-red-500 p-6 rounded-r-xl shadow-sm mb-6 flex items-start gap-4">
                <AlertTriangle className="text-red-500 shrink-0 mt-1" />
                <div>
                    <h2 className="text-lg font-bold text-red-700">Order Cancelled</h2>
                    <p className="text-red-600 text-sm mt-1">This order was cancelled. Any amount paid will be refunded to your source account within 5-7 business days.</p>
                </div>
            </div>
        ) : (
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 mb-4">
          <h2 className="text-2xl font-bold text-gray-900 mb-1">
            {order.status === 'Delivered' ? 'Order Delivered' : order.status === 'Cancelled' ? 'Cancelled' : order.status === 'Out for Delivery' ? 'On the way' : 'Preparing your order'}
          </h2>
          <p className="text-gray-500 mb-6">
            {order.status === 'Delivered' ? 'Enjoy your fresh produce!' : order.status === 'Cancelled' ? 'Order has been cancelled.' : 'Our delivery partner is riding responsibly.'}
          </p>
